...
\paragraph{Reproducibility note.} The Learned and RoPE perplexity values in Table~\ref{tab:controlled_ppl} reproduce from the released checkpoints to two decimal places (110.17 and 88.14 at $L{=}512$), confirming that the evaluation pipeline (tokenization, sliding windows, and perplexity computation) is faithful. The ALiBi checkpoint in this release reproduces at 126.93 at $L{=}512$, which is higher than the previously reported value of 93.7; because the identical pipeline exactly reproduces the other two methods, this discrepancy is attributed to the saved ALiBi checkpoint differing from the checkpoint used in the original run rather than to an evaluation artifact. All reported values in Table~\ref{tab:controlled_ppl} are measured from the released checkpoints, and all statistical claims use bootstrap CIs computed on the per-window data.
...
Within the training window ($L{=}512$), RoPE achieves the lowest perplexity (88.1, CI $[84.4,92.0]$), followed by Learned (110.2) and ALiBi (126.9, CI $[121.9,132.1]$). The RoPE--ALiBi gap at $L{=}512$ is $-38.8$ perplexity points (95\% bootstrap CI $[-40.4,-37.3]$, 482 windows, 1,000 resamples); the interval excludes zero and the RoPE--ALiBi ranking is statistically unambiguous at every context length, since no pair of intervals overlaps. As context length increases beyond the training window, Learned and RoPE degrade rapidly: Learned reaches 307.2 at $L{=}8192$ ($2.8\times$ the training-window value) and RoPE reaches 310.4 ($3.5\times$). ALiBi, by contrast, maintains nearly flat perplexity across the full range (126.9$\to$124.1, a $-2.2\%$ change; Figure~\ref{fig:extrapolation}). The extrapolation ranking reproduces the literature ranking from Section~\ref{sec:related} (ALiBi $\succ$ RoPE $\succ$ Learned) in terms of \emph{stability} at long contexts. Notably, the in-window absolute level of the ALiBi checkpoint reproduced here (126.9) is higher than previously reported (93.7); see the reproducibility note in Section~\ref{sec:controlled}.
...
Four limitations bound these results. First, the controlled experiment uses a relatively small model (17.7M parameters) and limited training data ($\sim$13M tokens); larger models trained on more data may exhibit different extrapolation characteristics. Second, we evaluate only vanilla RoPE without interpolation-based extensions (Position Interpolation, YaRN, NTK-aware scaling); these methods are designed to recover extrapolation capability and would likely improve RoPE's long-context performance. Third, the needle retrieval task produces no signal at this model scale (0\% across all methods), limiting its diagnostic value. Fourth, confidence intervals are reported using bootstrap resampling of validation windows, which quantifies within-run variability but not across-run reproducibility; in particular, the ALiBi in-window absolute perplexity differs between checkpoint copies, highlighting the importance of checkpoint provenance.
\section{Conclusion}
\label{sec:conclusion}

This paper presented a controlled empirical comparison of three positional encoding paradigms---Learned Absolute, RoPE, and ALiBi---using identical transformer architectures trained from scratch on WikiText-103. Our experiments yield three key findings. First, within the training window, RoPE achieves the lowest perplexity (88.1), followed by Learned (110.2) and ALiBi (126.9). Second, at $16\times$ the training length, learned and RoPE degrade sharply ($2.8\times$ and $3.5\times$ perplexity growth respectively), while ALiBi maintains the most stable perplexity (126.9$\to$124.1, $-2.2\%$), confirming that ALiBi provides genuine length extrapolation without fine-tuning. Third, needle retrieval fails uniformly (0\%) at this model scale, indicating that retrieval is capacity-bound rather than a positional encoding effect; we caution against interpreting needle results at small scale as evidence about extrapolation.
