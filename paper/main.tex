...
This paper compares three paradigms under controlled conditions. We keep model size, training data, and architecture fixed, training identical transformers (17.6--17.7M parameters) from scratch on WikiText-103 at a fixed context length of 512 tokens, differing only in their positional encoding. Our controlled comparison covers seven from-scratch encodings (Learned Absolute, Sinusoidal, RoPE, ALiBi, KERPLE, CABLE, and NoPE) plus two inference-time RoPE-scaling extensions (Position Interpolation and YaRN) evaluated on the vanilla RoPE checkpoint. We additionally evaluate three pre-trained models zero-shot (GPT-2, Pythia-160M, BLOOM-560M) to contextualize findings against real-world deployments.

The contributions of this paper are:
\begin{itemize}
\item A structured review of absolute, rotary, and bias-based positional encoding paradigms.
\item A controlled experiment isolating the positional encoding as the sole variable: identical transformer architectures trained on the same data for each of seven encodings (plus two RoPE-scaling extensions), evaluated at context lengths from 512 to 8,192 tokens ($16\times$ the training length), with bootstrap confidence intervals on every perplexity value.
...
\paragraph{Setup.} Identical transformer architectures (17.6--17.7M parameters: $d{=}256$, 6 layers, 4 heads, $d_{\text{head}}{=}64$, $d_{\text{ff}}{=}1024$, vocab 50,257) train from scratch on WikiText-103 ($\sim$13.0M tokens) at a fixed context length of 512 tokens for 10,000 steps. The only difference between models is the positional encoding: Learned Absolute, Sinusoidal, RoPE, ALiBi, KERPLE, CABLE, or NoPE. Position Interpolation and YaRN are evaluated as inference-time extensions of the trained RoPE checkpoint with no additional training. Training uses AdamW (lr $3{\times}10^{-4}$, weight decay 0.1, warmup 1000 steps, cosine decay), fp16 mixed precision, batch size 16 (micro-batch 8, gradient accumulation 2), seed 42 per method. We save checkpoints every 1,000 steps and evaluate the best validation perplexity checkpoint.
...
We compared seven positional encoding paradigms trained from scratch -- Learned Absolute, Sinusoidal, RoPE, ALiBi, KERPLE, CABLE, and NoPE -- together with two inference-time RoPE-scaling extensions (Position Interpolation and YaRN). We trained identical transformer architectures on WikiText-103, changing only the positional encoding. Our experiments yield three findings. First, within the training window, RoPE achieves the lowest perplexity (88.1), followed by Learned (110.2) and ALiBi (126.9). Second, at $16\times$ the training length, Learned and RoPE degrade sharply ($2.8\times$ and $3.5\times$ perplexity growth respectively), while ALiBi stays stable (126.9$\to$124.1, $-2.2\%$). ALiBi provides length extrapolation without fine-tuning in this setup. Third, needle retrieval fails uniformly (0\%) at this model scale. Retrieval failure is capacity-bound, independent of the positional encoding; we caution against reading needle results at small scale as evidence about extrapolation.
